...
def add_track_command(
    subparsers: argparse._SubParsersAction,  # type: ignore[type-arg]
) -> argparse.ArgumentParser:
    """Add the track command to the parser.

    This function adds a subparser for the track command (with tracking enabled).
    ...
    parser = subparsers.add_parser(
        "track",
        help="Run object tracking pipeline on videos.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.set_defaults(func=cli_run_split)
    _setup_parser(parser)
...
def cli() -> None:
    """Run the tracking pipeline CLI directly."""
    parser = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        description="Object Tracking Pipeline - Run RF-DETR detection and multi-object tracking on videos.",
    )
    _setup_parser(parser)
